...
inp = '''begfcd fabg aecgbdf cefagb edgcba eacbf efgbc bca ab decfa | cbgef befdcg bceaf fagb
...
L = inp.split("\n")
L = list(map(lambda x: x.split(), L))
NUM_WIRES = (2, 3, 4, 7) #Segments in wires
NUM_DISPLAY = (1, 7, 4, 8) #Number the segments correspond to
...
for x in range(len(L)):
    B = [0] * 10
    for y in range(10): # FINDS 1, 4, 7 ,8
        code = L[x][y]
        if len(code) in NUM_WIRES:
            B[NUM_DISPLAY[NUM_WIRES.index(len(code))]] = set(code)

    a = B[1] & B[7]
    for y in range(10): # FINDS 6
        code = L[x][y]
        if set(code) in B:
            continue
